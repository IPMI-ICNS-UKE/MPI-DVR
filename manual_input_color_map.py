...
class ColorMapSettingsView(Qt.QMainWindow):
    ...
    def adaptColormapMinMax(self):
        try:         
            ...
            self.parent().vtk_pip.updateColorMap(self.currentData, le_min_input, le_max_input)
    ...
    def functionCheckboxScalarBar(self):
        ...
        if not self.checkbox_scalar_bar.isChecked():    
            if self.currentData == 'bl':                          
                self.parent().vtk_pip.ren.RemoveActor2D(self.parent().vtk_pip.actor_scalar_bar_bl)
                if self.parent().vtk_pip.bool_scalar_bar_rm == True:
                    self.parent().vtk_pip.actor_scalar_bar_rm.SetPosition(0.03, 0.12)
                self.parent().vtk_pip.bool_scalar_bar_bl = False
            if self.currentData == 'rm':                          
                self.parent().vtk_pip.ren.RemoveActor2D(self.parent().vtk_pip.actor_scalar_bar_rm)
                self.parent().vtk_pip.bool_scalar_bar_rm = False
                if self.parent().vtk_pip.bool_scalar_bar_bl == True:
                    self.parent().vtk_pip.actor_scalar_bar_bl.SetPosition(0.03, 0.12)
            self.parent().iren.Initialize()
            self.parent().iren.Start()
        else:
            if self.currentData == 'bl':                          
                self.parent().vtk_pip.bool_scalar_bar_bl = True
            if self.currentData == 'rm':                            
                self.parent().vtk_pip.bool_scalar_bar_rm = True
            self.parent().vtk_pip.updateScalarBar(self.currentData, self.min_value, self.max_value) 
            self.parent().iren.Initialize()
            self.parent().iren.Start()             
...
class InputColorRGB(Qt.QMainWindow):
    ...
    def adaptRGBColor(self):
        ...
        try:           
            ...
            self.parent().vtk_pip.setMonoColor(self.currentData, r, g, b)  
        except: 
            print("Enter valid values!")
